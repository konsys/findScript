...
async function watcher() {
  ...
  for await (let proxy of Array(500).fill(1)) {
    try {
      page = await browser.newPage();
      console.log("Starting browser witn proxy", proxy);
      ...
      let rLinks = null;
      try {
        rLinks = JSON.parse(fs.readFileSync("./links.json"));
      } catch (err) {}

      if (!rLinks) {
        const ar = Array(50).fill(1);
        await sleep(3000);
        await page.setUserAgent(agents[randomInt(0, agents.length)]);
        await page.goto(
          `https://yandex.ru/efir?stream_active=blogger&stream_publisher=ugc_channel_5737249183219132976`,
          // idleParams,
        ...
        rLinks = await getYandexVideoLinks(page);
        fs.writeFileSync("./links.json", JSON.stringify(rLinks));
      }
      const links = sampleSize(rLinks, randomInt(8, 14));
      await page.close();
      for await (let link of links) {
        try {
          const page = await browser.newPage();
          await page.setUserAgent(agents[randomInt(0, agents.length)]);
          await page.goto(link, idleParams);
          await randSleep();
          await randSleep();
          await randSleep();
          // setTimeout(() => page.close(), randomInt(3, 6) * 60 * 1000);
        } catch (err) {
          console.log("proxy err", err);
        }
      }
      await sleep(randomInt(3, 6) * 60 * 1000);
      let pages = await browser.pages();
      for await (let pg of pages) {
        await pg.close();
      }
      console.log("close pages");
    } catch (err) {
      console.log("Proxy not ready", err);
      try {
      } catch (err) {
        //MOP
      }
